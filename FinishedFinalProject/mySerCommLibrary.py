...
# check if string is inside the list
def CheckList(l, target):
    for i in l:
        if(i == target):
            return True
    return False
...
def Watch4Box():
    if getSonicSensorCM() < 8:
        moveBackward(10)
        time.sleep(1.5)
        turnLeft(25)
        time.sleep(1)

def SleepingWatcher(sec: int):
	for i in range(1, 20 * sec):
		time.sleep(0.05)
		if getSonicSensorCM() < 15:
			return -1
		elif getSonicSensorCM() < 20:
			return 1
	return 0
